...
import AddUpazilaAgricultureOffice from "./AddUpazilaAgricultureOffice";
...
export default function DistrictList() {
    ...
    try {
      ...
      if (userRoleId === SUPER_ADMIN_ROLE_ID) {
        response = await axios.get(
          `${baseURL}/upazila-agriculture-offices/all`,
          AxiosHeader
        );
      } else {
        response = await axios.get(
          `${baseURL}/upazila-agriculture-offices/get-by-upazila/${userDistrictId}`,
          AxiosHeader
        );
      }
  ...
  const handleDeleteUserRole = async (id) => {
    const isDeleted = await DeleteAlert(
      id,
      "upazila-agriculture-offices/delete"
    );
    if (isDeleted) {
        ...
        >
          <div className="mb-6 flex flex-col lg:flex-row items-center justify-between gap-6">
            <div>
              ...
              >
                Agriculture Office List
              </Typography>
              <Typography color="gray" className="mt-1 font-normal">
                See information about all agriculture offices
              </Typography>
            </div>
            <div className="flex flex-col lg:flex-row items-center gap-4">
              ...
              <Button
                ...
                size="sm"
              >
                <PlusIcon strokeWidth={2} className="h-4 w-4" /> Add Agriculture
                Office
              </Button>
            </div>
          </div>
              ...
              <table className="w-full min-w-max table-auto text-left border-separate border-spacing-0">
                ...
                <tbody>
                  {currentTableData.map(
                    ({ district, name, status, createdAt, _id }, index) => {
                        ...
                        <tr key={_id} className="hover:bg-gray-50">
                          ...
                          <td className={classes}>
                            <div className="flex gap-2">
                              <Tooltip content="Edit Agriculture Office">
                                <IconButton
                                  onClick={() =>
                                    handleEditUserRole({
                                      _id,
                              ...
                              <Tooltip content="Delete Agriculture Office">
                                <IconButton
