...
import { AddGalleryFolder } from "./AddGalleryFolder";
...
export default function GalleryFolderList() {
  ...
  const handleDeleteBrand = async (id) => {
    const isDeleted = await DeleteAlert(id, "gallery-folders/delete");
    if (isDeleted) {
  ...
  const handleStatusChange = async (id, status) => {
    ...
    try {
      const response = await axios.patch(
        `${baseURL}/gallery-folders/status/${id}`,
        { status: !status },
        AxiosHeader
      );
          ...
          <div className="mb-6 flex flex-col lg:flex-row items-center justify-between gap-6">
            <div>
              ...
              <Typography color="gray" className="mt-1 font-normal">
                Manage all your gallery folder here
              </Typography>
            </div>
            <div className="flex flex-col lg:flex-row items-center gap-4">
              ...
              <Button
                ...
                size="sm"
              >
                <PlusIcon strokeWidth={2} className="h-4 w-4" /> Add Gallary
                Folder
              </Button>
            </div>
          </div>
              ...
              <table className="w-full min-w-max table-auto text-left border-separate border-spacing-0">
                ...
                <tbody>
                  {currentTableData.map(
                    ({ title, picture, createdAt, status, _id }, index) => {
      ...
      {openModal && (
        <Suspense fallback={<LazyLoader />}>
          <AddGalleryFolder
            ...
            existingGalleryFolder={selectedGalleryFolder}
          />
        </Suspense>
      )}
    </>
