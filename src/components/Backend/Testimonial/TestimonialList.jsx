...
export default function TestimonialList() {
  ...
  const handleDeleteUser = async (id) => {
    const isDeleted = await DeleteAlert(id, "testimonials/delete");
    if (isDeleted) {
  ...
  const handleStatusToggle = async (userId, currentStatus) => {
    try {
      ...
      const response = await axios.patch(
        `${baseURL}/testimonials/status/${userId}`,
        { status: newStatus },
        AxiosHeader
      );
          ...
          <div className="mb-6 flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4">
              ...
              <Button
                ...
                size="sm"
              >
                <PlusIcon strokeWidth={2} className="h-4 w-4" /> Add Testimonial
              </Button>
            </div>
          </div>
        </CardHeader>
              ...
              <table className="w-full min-w-max table-auto text-left border-separate border-spacing-0">
                ...
                <tbody>
                  {currentTableData.map(
                    (
                      { name, designation, message, picture, status, _id },
                      index
                    ) => {
                      ...
                      return (
                        <tr key={_id} className="hover:bg-gray-50">
                          ...
                          <td className={classes} style={{ width: "10px" }}>
                            <Typography
                              variant="small"
                              color="gray"
                              className="font-normal"
      ...
      <Suspense fallback={<LazyLoader />}>
        {openModal && (
          <AddTestimonial
            ...
            existingTestimonial={selectedUser}
          />
        )}
      </Suspense>
    </>
