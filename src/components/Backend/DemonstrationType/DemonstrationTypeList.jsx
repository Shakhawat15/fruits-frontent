...
import AddDemonstrationType from "./AddDemonstrationType";
...
export default function DemonstrationList() {
  ...
  const [selectedDemonstrationType, setSelectedDemonstrationType] =
    useState(null);
  ...
  const fetchDistrict = async () => {
    ...
    try {
      ...
      setDemonstrationType(response.data.data);
    } catch (error) {
  ...
  const handleOpenModal = (userRole = null) => {
    setSelectedDemonstrationType(userRole);
    setOpenModal(true);
  };

  const handleCloseModal = () => {
    setOpenModal(false);
    setSelectedDemonstrationType(null);
    fetchDistrict();
  };
  ...
  const handleDeleteUserRole = async (id) => {
    const isDeleted = await DeleteAlert(id, "demonstration-types/delete");
    if (isDeleted) {
      setDemonstrationType(DemonstrationType.filter((role) => role._id !== id));
    }
  };

  const totalPages = Math.ceil(DemonstrationType.length / itemsPerPage);
  ...
  const currentTableData = DemonstrationType.slice(
        ...
        >
          <div className="mb-6 flex flex-col lg:flex-row items-center justify-between gap-6">
            <div>
              ...
              >
                Demonstration Type List
              </Typography>
              <Typography color="gray" className="mt-1 font-normal">
                See information about all demonstration types
              </Typography>
            </div>
              ...
              <table className="w-full min-w-max table-auto text-left border-separate border-spacing-0">
                ...
                <tbody>
                  {currentTableData.map(
                    ({ title, status, createdAt, _id }, index) => {
                        ...
                        <tr key={_id} className="hover:bg-gray-50">
                          ...
                          <td className={classes}>
                            <div className="flex gap-2">
                              <Tooltip content="Edit Demonstration Type">
                                <IconButton
                                  onClick={() =>
                                    handleEditUserRole({
                                      _id,
                              ...
                              <Tooltip content="Delete Demonstration Type">
                                <IconButton
      ...
      {openModal && (
        <Suspense fallback={<LazyLoader />}>
          <AddDemonstrationType
            ...
            existingDemonstrationType={selectedDemonstrationType}
          />
        </Suspense>
      )}
    </>
