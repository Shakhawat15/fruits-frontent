...
export default function AddDistrict({ existingDistrict, onCancel }) {
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState(
    existingDistrict ? existingDistrict.status : false
  );
      ...
      try {
        ...
        if (existingDistrict) {
          response = await axios.put(
            `${baseURL}/districts/update/${existingDistrict["_id"]}`,
            formData,
            AxiosHeader
          );
        } else {
          response = await axios.post(
            `${baseURL}/districts/create`,
            formData,
            AxiosHeader
          );
        }
        if (response.status === 200 || response.status === 201) {
          SuccessToast(response.data.message || "District saved successfully");
          setTimeout(() => {
            onCancel(); // Close the modal after showing the toast
          }, 1000);
        } else {
          ErrorToast("Failed to save district");
        }
      } catch (error) {
    ...
    >
      ...
      <DialogHeader className="bg-gray-100 border-b border-gray-300">
        <h4 className="text-xl font-semibold text-gray-800">
          {existingDistrict ? "Update District" : "Add District"}
        </h4>
      </DialogHeader>
