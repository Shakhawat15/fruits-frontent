...
const TABLE_HEAD = ["S.No", "Title", "File", "Create Date", "Status", "Action"];

export default function PdfList() {
  const [openModal, setOpenModal] = useState(false);
  const [selectedPdf, setSelectedGalleryFolder] = useState(null);
  ...
  const fetchPdfs = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${baseURL}/pdfs/all`, AxiosHeader);
      setGalleryFolder(response.data.data);
    } catch (error) {
      ErrorToast("Failed to fetch PDF data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteBrand = async (id) => {
    const isDeleted = await DeleteAlert(id, "pdfs/delete");
    if (isDeleted) {
      setGalleryFolder(galleryFolder.filter((b) => b._id !== id));
    }
  };
  ...
  const handleStatusChange = async (id, status) => {
    ...
    try {
      const response = await axios.patch(
        `${baseURL}/pdfs/status/${id}`,
        { status: !status },
        AxiosHeader
      );
      if (response.status === 200) {
        SuccessToast("PDF status updated successfully");
        setGalleryFolder(
          galleryFolder.map((b) =>
  ...
  const totalPages = Math.ceil(galleryFolder.length / itemsPerPage);
  ...
  const currentTableData = galleryFolder.slice(
    ...
    <>
          ...
          <div className="mb-6 flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4">
              ...
              <Button
                ...
                size="sm"
              >
                <PlusIcon strokeWidth={2} className="h-4 w-4" /> Add PDF
              </Button>
            </div>
          </div>
        </CardHeader>
              ...
              <table className="w-full min-w-max table-auto text-left border-separate border-spacing-0">
                ...
                <tbody>
                  {currentTableData.map(
                    ({ title, file, createdAt, status, _id }, index) => {
      ...
      {openModal && (
        <Suspense fallback={<LazyLoader />}>
          <AddPdf onCancel={handleCloseModal} existingPdf={selectedPdf} />
        </Suspense>
      )}
    </>
  );
